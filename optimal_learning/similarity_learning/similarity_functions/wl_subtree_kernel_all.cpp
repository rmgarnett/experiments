...
{
	mwIndex *A_ir, *A_jc;
	double *graph_ind, *labels_in, *h_in, *kernel_matrix, *res;
	int h, *labels;
	...
	result          = new float[num_graphs * num_graphs];
	...
	while (true) {
		...
		feature_vectors = new float[num_graphs * num_labels]();

		for (i = 0; i < num_nodes; i++) 
			feature_vectors[INDEX(graph_ind[i] - 1, labels[i] - 1, num_graphs)]++;
	
		cblas_ssyrk(CblasColMajor, CblasUpper, CblasNoTrans, num_graphs, num_labels,
		 						1.0, feature_vectors, num_graphs, 0.0, result, num_graphs);
		
		for (j = 0; j < num_graphs; j++)
			for (i = j; i < num_graphs; i++) {
				index = INDEX(i, j, num_graphs);
				kernel_matrix[index] +=	result[index];
			}
